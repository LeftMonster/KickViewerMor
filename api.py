...
class KickPool:
    ...
    async def loop_update_streamer(self):
        ...
        while not self.streamers.empty():
            ...
            streamer = self.streamers.get(block=True, timeout=self.pause_interval)
            flag, channel_id = await check_online_status(config_oauth=self.stream_oauth,
...
class KickClientPool:
    ...
    async def connect_kick_viewer_ws(self, channel_id: int, token_list: list[str], livestream_id: int):
                    ...
                    for i in range(360):
                        try:
                            ...
                            if i % 24 == 0:
                                ...
                                await ws.send_json({"type": "channel_handshake",
                                                    "data": {"message": {"channelId": f"{str(channel_id)}"}}})
                                logger.info(f"i: {i}")
                                logger.info(">> Sent handshake:", handshake_msg)
                                time.sleep(0.1)
                                living_event = {"type": "user_event", "data": {
                                    "message": {"name": "tracking.user.watch.livestream", "channel_id": channel_id,
                                                "livestream_id": livestream_id}}}
                                logger.info(f">> Sent tracking event: {living_event}")
                                # ws.send_json(living_event, opcode=websocket.ABNF.OPCODE_TEXT)
                                await ws.send_json(living_event)
        ...
        for token in token_list:
            if token is not None:
                task = asyncio.create_task(connect_to_ws(token=token, headers=headers))
                fts.append(task)
                #fts.append(future_ws)
        result = await asyncio.gather(*fts, return_exceptions=True)
...
async def main():
    ...
    streamers_list = ['omni', 'oilrats', 'agustabell212', 'coconutb', 'welyn', 'templetaps', 'posty', 'trainwreckstv',
                      'lifestomper', 'qaixx', 'picco', 'panpots', 'hutnik', 'serwinter', 'dilanzito', 'spoonkid',
                      'winnie',
                      'mendo', 'hjune', 'blazed', 'ricoy', 'xqc']
