...
async def async_campaigns(oauth):
    ...
    async with AsyncRustTLSProxyClient(
        ...
        resp = await client.get(url, headers=headers)
        if resp.status_code != 200:
            logger.info(f"campaigns数据获取: {resp.status_code}")
        return resp.json()
...
async def init_channel_page(oauth, name):
    ...
    async with AsyncRustTLSProxyClient(
            base_url="http://127.0.0.1:5005",
            default_profile="chrome_138_windows",
    ) as client:
        resp = await client.get(url, headers=headers)
        if resp.status_code != 200:
            logger.info(f"homepage获取: {resp.status_code}")
            logger.error(resp.text)
            raise Exception("Risk warning")
        return resp.text
...
class KickPool:
    ...
    def __getitem__(self, item):
        if isinstance(item, int):
            ...
            if item in self.streamers_exist and item in self.streamers_list:
                ...
                #         return streamer
                # return None
                for streamer in self.streamers_normal:
                    if streamer.streamer_name == item:
                        return streamer
                return None
        else:
    ...
    async def run(self):
        for name in self.streamers_exist:
            ...
            self.streamers.put(streamer)
            self.streamers_normal.append(streamer)
    ...
    async def remove_streamer(self, name: str) -> bool:
        ...
        for index in range(self.streamers.qsize()):
            ...
            # TODO Ready to test
            self.streamers_normal.remove(streamer)
...
class KickClientPool:
    ...
    async def get_websocket_token(self) -> List[str]:
                ...
                async with AsyncRustTLSProxyClient(
                    ...
                    if token:
                        if response.status_code != 200:
                            logger.info(f"  ✗ Token获取失败: {token[:20]}...")
        ...
        logger.debug("获取WebSocket Token")
        url = "https://websockets.kick.com/viewer/v1/token"
        task_list = []
        batch = 50
        # total task
    ...
    @retry(exception_to_catch=Exception, num_times=3, delay_seconds=1)
    async def connect_kick_viewer_ws(self, channel_id: int, token_list: list[str], livestream_id: int, pool: KickPool, selected_name: str):
        ...
        def connect_to_ws(token, headers):
            ...
            # ========= 开始接收服务器推送 =========
            for i in range(360):
                if selected_name in pool.streamers_list:
                    for streamer in pool.streamers_normal:
                        if streamer.streamer_name == selected_name and not streamer.online:
                            logger.warning("当前主播: {}, 已离线, 观看终止".format(selected_name))
                            break
                try:
                    ...
                    if i % 24 == 0:
                        ...
                        ws.send(json.dumps({"type": "channel_handshake", "data": {"message": {"channelId": f"{str(channel_id)}"}}}))
                        # ws.send(json.dumps(handshake_msg))
                        logger.info(f"i: {i}")
                        logger.info(">> Sent handshake: {}".format(json.dumps({"type": "channel_handshake", "data": {"message": {"channelId": f"{str(channel_id)}"}}})))
                        # time.sleep(0.1)
    ...
    async def run_auto_drops_watcher(self, kick_pool):
        ...
        while True:
            try:
                # ===== 步骤1: 查询进度并生成队列 =====
                if not kick_pool.init_finished:
                #if kick_pool.streamers.qsize() == 0 or not kick_pool.init_finished:
                    if kick_pool.streamers.qsize() == 0:
                        logger.info("主播信息还在加载中...")
                    await asyncio.sleep(15)
                    continue
                else:
                    logger.info("[队列更新] 查询drops进度...")
                ...
                for kick_streamer in kick_pool.streamers_normal:
                    temp_queue.append(kick_streamer)
                    if kick_streamer.online:
                        cid = getattr(kick_streamer, 'channel_id', None)
                        lid = getattr(kick_streamer, 'livestream_id', None)
                        if cid and lid:
                            online_streamers[kick_streamer.streamer_name.lower()] = (cid, lid)
                ...
                await self.connect_kick_viewer_ws(
                    ...
                    pool=kick_pool,
                    selected_name=next_task.selected_streamer
                )
                ...
                for check_count in range(3):  # 最多观看1小时
                    ...
                    # 检查主播是否还在线
                    for kick_streamer in kick_pool.streamers_normal:
                        if kick_streamer.streamer_name.lower() == next_task.selected_streamer:
                            if not kick_streamer.online:
